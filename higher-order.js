...
//Code Here
let totalPopulation = populations.reduce(function(preVal,Current){
...
*/
//  let newPurchases= purchases.filter(({'owner'}) => 'owner'=== 'Bob')
let bobsTotal = purchases.reduce((preVal, current)=> {
if(current['owner'] === 'Bob'){
  return preVal + current['price']
}else{
  return preVal
}
}, 0)
